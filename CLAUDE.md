...
- **House primitives — use in every new site, not just this one**: `SplineScene` (`components/spline-scene.tsx`) and `ContainerScroll` (`components/motion/container-scroll.tsx`). Both are part of the studio's standard kit going forward — wire them into the appropriate moment on each new build (Spline for a 3D editorial mark or hero element; ContainerScroll for a featured case study / screen reveal). Don't ship a site without considering where each one earns its place.
